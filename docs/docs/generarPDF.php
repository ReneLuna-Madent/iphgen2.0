...
$pdf->SetLineWidth(8.0);
$pdf->SetDrawColor(214,4,4);
$pdf->SetFont('Arial', '', '9');
...
$invd = str_split(str_replace("", "", $_POST['linea1000']));
...
//Equis no se entregan anexos
if (isset($_POST["anexoNo"])) {
    $pdf->SetXY(182, 103.3);
    $pdf->Write(0, 'X');
}
...
//Denuncia
if (isset($_POST["comoSeEnteroDelEcho"])&&$_POST["comoSeEnteroDelEcho"]=="denuncia") {
    $pdf->SetXY(49.5, 104);
    $pdf->Write(0, 'X');
}
//Flagrancia
if (isset($_POST["comoSeEnteroDelEcho"])&&$_POST["comoSeEnteroDelEcho"]=="flagrancia") {
    $pdf->SetXY(93.5, 104);
    $pdf->Write(0, 'X');
}
//Localizacion
if (isset($_POST["comoSeEnteroDelEcho"])&&$_POST["comoSeEnteroDelEcho"]=="localizacion") {
    $pdf->SetXY(134, 104);
    $pdf->Write(0, 'X');
}
//Mandamiento judicial
if (isset($_POST["comoSeEnteroDelEcho"])&&$_POST["comoSeEnteroDelEcho"]=="MandamientoJudicial") {
    $pdf->SetXY(190, 104);
    $pdf->Write(0, 'X');
}
//Llamada de emergencia
if (isset($_POST["comoSeEnteroDelEcho"])&&$_POST["comoSeEnteroDelEcho"]=="LlamadaDeEmergencia") {
    $pdf->SetXY(49.5, 109);
    $pdf->Write(0, 'X');
}
//Descubrimiento
if (isset($_POST["comoSeEnteroDelEcho"])&&$_POST["comoSeEnteroDelEcho"]=="descubrimiento") {
    $pdf->SetXY(93.5, 109);
    $pdf->Write(0, 'X');
}
//Aportacion
if (isset($_POST["comoSeEnteroDelEcho"])&&$_POST["comoSeEnteroDelEcho"]=="aportacion") {
    $pdf->SetXY(134, 109);
    $pdf->Write(0, 'X');
}
//911 No.
...
//Longitud
$lond = str_replace(".", "", $_POST['linea48']);
$lond = str_split(str_replace("-", "", $lond));
...
if(strlen($_FILES["archivo"]["name"])>3){
  $pdf->Image($_FILES["archivo"]["tmp_name"], 12,41,190,150,$extencion);  
}
...
$posicion = 31;
$narrd = explode("\n",utf8_decode( $_POST['narrativaHech']), 110);
...
$posicion = 142;
$narrd = explode("\n",utf8_decode($_POST['descripcionPerDet']),88);
for ($i = 0; $i < count($narrd); $i++) {
    //$posicion=$posicion+7;
...
if (isset($_POST["ObjetosEncontrados"])&&($_POST["ObjetosEncontrados"]=="si")) {
    $pdf->SetXY(32.5, 117);
    $pdf->Write(0, 'X');
}
if (isset($_POST["ObjetosEncontrados"])&&($_POST['ObjetosEncontrados']=="no")) {
    $pdf->SetXY(97.5, 117);
    $pdf->Write(0, 'X');
}
...
$pdf->Output($nombre, 'F');
$pdf->Output($nombre, 'D');
#rename($_FILES["video"]["tmp_name"],"carpeta/otra/".$_FILES["video"]["name"]);